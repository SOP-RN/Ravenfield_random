...
function dvCCam:CalculateDynamicFov()
    ...
    return math.min(baseFov * (baseDistance / distance),100)
end
...
function dvCCam:ApplyCameraStyle()
    ...
    elseif self.cameraStyle == 2 then -- Fly By
        ...
        tpCamera.fieldOfView = self:CalculateDynamicFov()
...
function dvCCam:NewFlyByPosition()
    if self.attractModeTarget ~= nil then
        ...
        local randomOffset = Vector3(Random.Range(-50, 50), Random.Range(-50, 50), Random.Range(-55, 55))
        
        -- Bias the position towards the movement direction
        local biasedPosition = self.attractModeTarget.position + movementDirection * 300 + randomOffset
